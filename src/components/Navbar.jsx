...
function Navbar({ authorName, books, upcoming, searchTerm, setSearchTerm }) {
  ...
  return (
    <>
      ...
      {/* Main navbar bar */}
      <header className="bg-midnight-navy border-b border-midnight-navy">
      ...
      {/* Mobile menu — sibling to top bar div, inside nav, normal flow */}
      {menuOpen && (
          <div className="md:hidden border-t border-[rgba(232,196,104,0.2)] bg-[#1a1a2e] py-2">
            {/* Home */}
            <NavLink
              to="/"
              end
            ...
            {/* Books row — text + chevron centered together */}
            <div className="flex items-center justify-center py-2 gap-1">
              <button
                type="button"
                onClick={() => { navigate('/books'); setMenuOpen(false); }}
                className="text-center text-[15px] tracking-[0.03em] text-[#f5f0e8] transition hover:text-[#e8c468] px-2"
              >
                Books
              </button>
              <button
